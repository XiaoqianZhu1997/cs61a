...
class Player(object):
    ...
    def go_to(self, location):
        ...
        # 这个是class Place的一个instance，这样就把目的地用class的形式表示
        ...
        """
            print('You are at' + ' ' + self.place.name)
        elif destination_place.name == self.name:
            print("Can't go to" + str(self.place.name) + 'from' + str(self.place.name) + '.')
            print('Try to looking around to see where to go.')
            print('You are at' + ' ' + self.place.name)
        else:
            self.place = destination_place
            print('You are at' + ' ' + str(destination_place.name))
        """
    ...
    def talk_to(self, person):
        ...
        if type(person) != str:
            ...
            d = self.place.characters
            if person in d:
                print(str(person) + ' is not here.')
            else:
                print(str(person) + ' says: ' + str(d[person].message))
    ...
    def take(self, thing):
        ...
        if type(thing) != str:
            ...
            d = self.place.things  # dictionary
            if thing not in d:
                print(str(thing) + ' is not here.')
            else:
                print('Player takes the ' + str(thing))
                self.backpack.append(d[thing])
                d.pop(thing)
                # 之前上面两行顺序写反了会出现"KeyError"，因为已经将Key删除了，无法indexing
