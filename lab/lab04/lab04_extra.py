...
def merge(lst1, lst2):
    ...
    if lst1 == []:
        ...
        element1 = [i for i in lst1 if i <= lst2[0]]
        element2 = [i for i in lst1 if i > lst2[0]]
        lst0 = element1 + [lst2[0]] + element2
        return merge(lst0, lst2[1:])
...
def replace_elem(lst, index, elem):
    ...
    "*** YOUR CODE HERE ***"
    element1 = lst[: index]
    element2 = lst[index+1: ]
    return element1 + [elem] + element2 
...
def put_piece(board, max_rows, column, player):
    ...
    for i in range(max_rows):
        if board[max_rows-1-i][column] == '-':
            row = max_rows -1 - i
            new_list = replace_elem(board[max_rows-1-i], column, player)
            new_board = replace_elem(board, max_rows-1-i, new_list)
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    if col not in range(max_cols):   # 要保证valid
        return -1, board
    else:
        for i in range(max_rows):
            if board[max_rows-1-i][col] == '-':
                row = max_rows -1 - i
                new_list = replace_elem(board[max_rows-1-i], col, player)
                new_board = replace_elem(board, max_rows-1-i, new_list)
                return row, new_board
        return -1, board
...
def print_board(board, max_rows, max_cols):
    ...
    for i in range(max_rows):
        ...
        for j in range(max_cols):
            print_row = print_row + row[j] + ' ' # 要注意test里面的空格
        strip_row = print_row.strip()  # 去掉首尾的空格
        print(strip_row)
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    "*** YOUR CODE HERE ***"
    adjacent,col = 0, 0
    while col < max_cols:  # 把一行中的从第一列到最后一列开始找
        piece = get_piece(board, row, col)
        if piece == player:
            adjacent = adjacent + 1
        else:
            adjacent = 0  # 只要有一次不连续就会变成0
        col = col + 1
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    "*** YOUR CODE HERE ***"
    adjacent,rows = 0, 0
    while rows < max_rows:   # 把一列中的从第一行开始找到最后一行
        piece = get_piece(board, rows, col)
        if piece == player:
            adjacent = adjacent + 1
        else:
            adjacent = 0  # 只要有一次不连续就会变成0
        ...
        if adjacent >= num_connect:
            return True
    return False   # while循环遍历了所有列/行都没有连续的，return False
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    diagonal_win = check_win_diagonal(board, max_rows, max_cols, num_connect,
                                      row, col, player)
    "*** YOUR CODE HERE ***"
    result = check_win_row(board, max_rows, max_cols, num_connect, row, player) or check_win_column(board, max_rows, max_cols, num_connect, col, player) or check_win_diagonal(board, max_rows, max_cols, num_connect, row, col, player)
    return result
...
def check_win_diagonal(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    while row_top_left > 0 and col_top_left > 0:
        row_top_left -= 1
        col_top_left -= 1  # ⚠️这里循环完后的参数值是可以传入下面的while中的
