...
def group_by_first(pairs):
    ...
    keys = []
    for key, _ in pairs:
        if key not in keys: # 得到了所有unique key
            keys.append(key)  # append就是添加一个元素
...
def group_by_centroid(restaurants, centroids):
    ...
    # 从restaurants里面挨个选择，找到最近的centroid，然后返回一个lists，其中每个元素是个pairs，【centroid，restaurant】
...
def find_predictor(user, restaurants, feature_fn):
    ...
    b = S_xy / S_xx
    a = mean(ys) - b * mean(xs) # REPLACE THIS LINE WITH YOUR SOLUTION
...
def rate_all(user, restaurants, feature_fns):  # 只有一个user，所以是每个restaurant在dicy的keys中只会出现一次
    ...
    user_reviewed_restaurants输出的是个list，其中元素是restaurants
    hint: user_rating 输入值是user和restaurant_name，输出的是rating
    """
